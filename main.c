...
xQueueHandle Traffic_Light_Colour = 0;
...
int main(void) {
	...
	Traffic_Light_Colour = xQueueCreate(mainQUEUE_LENGTH, sizeof(uint32_t));
	...
	while (1) {
		if ( xQueueReceive(Traffic_Flow_Queue, &flow_rate, 500) ) {
			...
			if (flow_rate >= MAX_TRAFFIC) {
				...
				if (rounds_since_last_added_car > 5  && rand() % 2 == 0){
					traffic_to_display = shift_traffic(ADD_CAR);
					rounds_since_last_added_car = 0;
	...
	while(1)
	{
		...
		if (xQueueReceive(Traffic_Flow_Queue, &flow_rate, 500)) {
			if (xTimerIsTimerActive(Traffic_Light_Timer) == pdFALSE) {
				if (xQueueReceive(Traffic_Light_Colour, &light_colour, 500)) {
					if (light_colour == GREEN_LIGHT) {
						// want a constant delay for yellow light
						xTimerChangePeriod(Traffic_Light_Timer, pdMS_TO_TICKS(2000), 0);
					}
					else {
...
uint32_t shift_traffic(int add_car) {
	...
	if (xQueueReceive(Prev_Traffic_Queue, &prev_traffic, 500)) {
		...
		if(new_light_colour != light_colour) {
			// write the traffic light colour to traffic light colour queue, to be used by traffic light task
			xQueueSend(Traffic_Light_Colour, &new_light_colour, 1000);
		}
